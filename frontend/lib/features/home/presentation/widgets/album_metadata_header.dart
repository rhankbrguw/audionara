...
class AlbumMetadataHeader extends StatelessWidget {
  ...
  Widget _buildMetaChips() {
    final count = album.tracks.isNotEmpty ? album.tracks.length : album.meta.trackCount;
    final typeLabel = _formatRecordType(album.meta.recordType, count);
    final countLabel = GeneralStrings.songCount(count);
    final chips = <Widget>[
      _buildPill(Icons.album_outlined, GeneralStrings.recordTypeWithCount(typeLabel, countLabel)),
      if (album.meta.genre.isNotEmpty) _buildPill(Icons.music_note_rounded, album.meta.genre),
      if (album.totalDurationMs > 0) _buildPill(Icons.schedule_rounded, formatDuration(album.totalDurationMs)),
    ];
  ...
  String _formatRecordType(String recordType, int trackCount) {
    ...
    if (lower == 'single' || (trackCount > 0 && trackCount <= 3)) return GeneralStrings.recordTypeSingle;
    if (lower == 'ep' || (trackCount >= 4 && trackCount <= 6)) return GeneralStrings.recordTypeEp;
    if (lower == 'compile' || lower.contains('compilation')) return GeneralStrings.recordTypeCompilation;
    return GeneralStrings.recordTypeAlbum;
  }
