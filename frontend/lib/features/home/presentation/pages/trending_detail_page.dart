...
class _TrendingDetailPageState extends State<TrendingDetailPage> {
  ...
  void _fetchTracks() {
    ...
    final isMix = widget.vibeName == 'MixForYou' || widget.title == HomeStrings.mixForYouTitle;
    final future = isMix ? bloc.fetchMixForYouUseCase() : bloc.searchByVibeUseCase(widget.title);
    _tracksFuture = future.then((tracks) {
      final seen = <String>{};
  ...
  void _playAll(List<Track> queue) {
    if (queue.isNotEmpty) _playTrack(queue.first, queue);
  }
  ...
  Widget _buildErrorScaffold(String error) {
    final clean = error.contains('<') ? GeneralStrings.networkErrorPrefix : error;
    return Scaffold(
      backgroundColor: AppColors.background,
      appBar: AppBar(
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            Text(clean, style: const TextStyle(color: AppColors.textInverse, fontSize: 13)),
