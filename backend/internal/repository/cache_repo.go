...
type CacheRepository interface {
	...
	InvalidateStreamURL(ctx context.Context, artist, title string) error
...
// InvalidateStreamURL removes cached audio stream from L1 and Redis L2.
func (r *redisCacheRepo) InvalidateStreamURL(ctx context.Context, artist, title string) error {
	key := "stream:" + strings.ToLower(strings.TrimSpace(artist)) + ":" + strings.ToLower(strings.TrimSpace(title))
	r.l1Cache.Delete(key)
	return r.client.Del(ctx, key).Err()
}

// InvalidateDevice purges personalized recommendation caches for a device.
func (r *redisCacheRepo) InvalidateDevice(ctx context.Context, deviceID string) error {
	if strings.TrimSpace(deviceID) == "" {
		return nil
	}
	return r.client.Del(ctx, "mix:"+deviceID, "home:feed:"+deviceID).Err()
}
