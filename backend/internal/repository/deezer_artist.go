...
func (r *DeezerRepository) gatherArtistContent(ctx context.Context, id, name string) ([]*domain.Track, []*domain.AlbumMeta, string, string) {
	...
	wg.Add(3)
	go func() { defer wg.Done(); topTracks, _ = r.GetArtistTopSongs(ctx, id, true) }()
	go func() { defer wg.Done(); albums = r.fetchArtistAlbums(ctx, id, name) }()
	go func() { defer wg.Done(); bio, history = r.fetchWikiSummary(ctx, name) }()
	wg.Wait()
	return topTracks, albums, bio, history
}

func (r *DeezerRepository) fetchFullArtistDetail(ctx context.Context, artistID string) (*domain.ArtistDetail, error) {
	targetID := r.resolveTargetArtistID(ctx, artistID)
	var artistDTO deezerArtistDTO
	if err := r.fetchJSON(ctx, "/artist/"+targetID, &artistDTO); err != nil {
		return nil, fmt.Errorf("deezer artist lookup: %w", err)
	}

	actualID := strconv.FormatInt(artistDTO.ID, 10)
	topTracks, albums, bio, history := r.gatherArtistContent(ctx, actualID, artistDTO.Name)
	if len(topTracks) == 0 {
		topTracks = fetchArtistTopSongsFromITunes(ctx, artistDTO.Name)
	}
	if len(albums) == 0 {
		albums = fetchArtistAlbumsFromITunes(ctx, artistDTO.Name)
	}
	...
	return &domain.ArtistDetail{
		...
		Genres:     extractArtistGenres(albums),
