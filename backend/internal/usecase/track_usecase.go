...
// LogPlay logs a track play for a device and invalidates recommendation caches.
func (uc *TrackUseCase) LogPlay(ctx context.Context, record *domain.HistoryRecord) error {
	if err := uc.historyRepo.LogPlay(record); err != nil {
		return err
	}
	if uc.cacheRepo != nil && record != nil && record.DeviceID != "" {
		_ = uc.cacheRepo.InvalidateDevice(ctx, record.DeviceID)
	}
	return nil
}
