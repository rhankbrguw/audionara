...
func (uc *TrackUseCase) GetYouTubeStreamURL(ctx context.Context, artist, title string, explicit bool, quality int) (string, error) {
	...
	return uc.flight.Do(flightKey, func() (string, error) {
		...
		return uc.resolveAndCacheStream(ctx, artist, cleanTitle, explicit)
	})
}

// InvalidateStreamURL removes cached audio stream for this artist and title.
func (uc *TrackUseCase) InvalidateStreamURL(ctx context.Context, artist, title string) {
	if uc.cacheRepo != nil {
		_ = uc.cacheRepo.InvalidateStreamURL(ctx, artist, normalizeTrackTitle(title))
	}
}

// GetFreshStreamURL resolves fresh audio bypassing stale caches.
func (uc *TrackUseCase) GetFreshStreamURL(ctx context.Context, artist, title string, explicit bool) (string, error) {
	cleanTitle := normalizeTrackTitle(title)
	uc.InvalidateStreamURL(ctx, artist, title)
	return uc.resolveAndCacheStream(ctx, artist, cleanTitle, explicit)
}

func (uc *TrackUseCase) resolveAndCacheStream(ctx context.Context, artist, cleanTitle string, explicit bool) (string, error) {
...
func (uc *TrackUseCase) resolveStreamWithFallbacks(ctx context.Context, artist, cleanTitle string, explicit bool) (string, bool) {
	if url, err := resolveStudioSaavnAudio(ctx, artist, cleanTitle); err == nil && url != "" {
		return url, true
	}
	...
	if url, err := resolveAudioStreamViaITunes(ctx, artist, cleanTitle); err == nil && url != "" {
		return url, false
	}
	return "", false
}
	...
	go func(items []*domain.Track) {
		...
		for _, track := range items {
			...
			go func() {
				...
				durSec := fmt.Sprintf("%d", t.DurationMs/1000)
				_, _ = uc.GetLyrics(bgCtx, t.Artist, t.Title, durSec)
			}()
		}
	}(toFetch)
}
